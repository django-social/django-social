...
for library in [LIBRARY_TYPE_VIDEO, LIBRARY_TYPE_IMAGE, LIBRARY_TYPE_AUDIO]:
    ...
    url(r'^%s/add/$' % library, 'file_edit', name='%s_add' % library, kwargs=params),
    url(r'^%s/(?P<id>[a-f0-9]{24})/add/$' % library, 'file_edit', name='%s_add' % library, kwargs=params),
    url(r'^%s/(?P<file_id>[a-f0-9]{24})/edit/$' % library, 'file_edit', name='%s_edit' % library, kwargs=params),
